...
import org.iota.jota.model.Signature;
...
/**
 * 
 * Utility functions over the default API calls from the node
 *
 */
public class IotaAPIExtended extends IotaAPICore {
    
    private static final Logger log = LoggerFactory.getLogger(IotaAPIExtended.class);
    
    protected <T extends Builder<T, E>, E extends IotaAPICore> IotaAPIExtended(Builder<T, E> builder) {
        super(builder);
    }
    ...
    public GetNewAddressResponse generateNewAddresses(String seed, int security, boolean checksum, int index, int amount, boolean addSpendAddresses) throws ArgumentException {
        ...
        //Make sure that when requesting 0 new ones, but want spent addresses, we loop until first unspent.
        
        boolean firstEmpty = true;
        for (int i = index, numUnspentFound=0; numUnspentFound < amount && firstEmpty == false; i++) {
    ...
    public GetAccountDataResponse getAccountData(String seed, int security, int index, boolean checksum, int total, boolean returnAll, int start, int end, boolean inclusionStates, long threshold) throws ArgumentException {
        ...
        GetNewAddressResponse gna = generateNewAddresses(seed, security, checksum, index, total, returnAll);
    ...
    public void validateTransfersAddresses(String seed, int security, List<String> trytes) throws ArgumentException {
        ...
        GetNewAddressResponse gna = generateNewAddresses(seed, security, false, 0, 0, true);
     ...
     * @param validateInputs     Whether or not to validate the balances of the provided inputs.
     * @param validateInputAddresses  Whether or not to validate if the destination address is already used, if a key reuse is detect or it's send to inputs.
     * @param tips               The starting points we walk back from to find the balance of the addresses
    ...
    /**
     * Internal function to get the balances and format the output.
     *
     * @param addresses The addresses.
     * @param tips      The starting points we walk back from to find the balance of the addresses
     * @param threshold Min balance required.
     ...
     **/
    protected GetBalancesAndFormatResponse getBalanceAndFormat(final List<String> addresses, final List<String> tips, long threshold, int start, StopWatch stopWatch, int security) throws ArgumentException, IllegalStateException {
